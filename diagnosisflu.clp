...
(defrule Awalan
    =>
    (printout t crlf "Apakah Anda ingin mendiagnosis gejala flu?" crlf "0: Tidak" crlf "1: Ya" crlf)
    (bind ?x (read))
        (if (= ?x 1)
...
(defrule tipeA (gejalaUmum yes)
	...
	(bind ?x (read))
        (if (= ?x 1)
			then (printout t crlf "Dari gejala yang Anda alami, sistem mendiagnosis bahwa Anda mengalami Flu Tipe A." crlf "Perbanyak istirahat dan konsumsi obat sesusai resep" crlf "Jika tidak kunjung membaik selama lebih dari seminggu, segera hubungi dokter." crlf)
        )
        (if (= ?x 0)
...
(defrule tipeA (tipeA no)
	...
	(bind ?x (read))
        (if (= ?x 1)
			then (printout t crlf "Dari gejala yang Anda alami, sistem mendiagnosis bahwa Anda mengalami Flu Tipe B." crlf "Perbanyak istirahat dan minum air putih, konsumsi obat, serta jaga jarak dengan orang lain" crlf "Jika tidak kunjung membaik selama lebih dari seminggu, segera hubungi dokter." crlf)
        )
        (if (= ?x 0)
...
(defrule tipeFB (tipeB no)
	...
	(bind ?x (read))
        (if (= ?x 1)
			then (printout t crlf "Dari gejala yang Anda alami, sistem mendiagnosis bahwa Anda mengalami Flu Burung." crlf "Harap segera mengunjungi dokter dan mendapatkan perawatan intensif." crlf)
        )
        (if (= ?x 0)
...
(defrule tipeTL (tipeFB no)
	...
	(bind ?x (read))
        (if (= ?x 1)
			then (printout t crlf "Dari gejala yang Anda alami, sistem mendiagnosis bahwa Anda mengalami Flu Tulang." crlf "Perbanyak istirahat dan minum air putih, serta konsumsi obat penurun demam." crlf "Jika tidak kunjung membaik, segera hubungi dokter." crlf)
        )
        (if (= ?x 0)
